...
uint64_t archive_offset = sizeof(PreHeader); uint64_t original_offset = 0;
...
task->b = b; task->gap_len = gap; task->gap_archive_offset = archive_offset;
archive_offset += gap; task->o_pos_start = original_offset;
...
safe_temp.data(), rec_level, &opts, nullptr, nullptr, 0
);
if (comp_res <= 0) {
task->fatal_error = true; task->error_msg = "Oodle Re-compression failed."; return task;
}
...
uint32_t crc_stored = CalculateCRC32(task->block_data_in.data(), task->b.stored_size);
if (crc_stored != task->b.crc32) {
task->fatal_error = true; task->error_msg = "CRC32 Integrity Check Failed for raw block."; return task;
}
dec_fails++;
return task;
}));
...
dec_queue.pop();
if (!process_and_write(completed)) { fatal_abort = true; break; }
processed_blocks++;
...
auto completed = dec_queue.front().get();
dec_queue.pop();
if (!fatal_abort && !process_and_write(completed)) { fatal_abort = true; }
}
