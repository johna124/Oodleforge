...
Result<int> RunEncode(const std::string& input_path, const std::string& output_path, bool verbose, int num_threads, const std::vector<int32_t>& m_ids, const std::vector<int32_t>& opt_levels, bool opt_auto, bool opt_force, const std::vector<uint8_t>& aesKey, bool useAES, uint32_t tradeoff_bytes, bool quantum_crc) {
...
PreHeader hdr{};
hdr.magic = 0x50524546; hdr.version = 33; hdr.original_size = fSize; hdr.block_count = 0; hdr.use_aes = 0;
if (useAES) {
hdr.use_aes = 1;
...
uint64_t pos = 0; uint64_t last_out = 0;
std::vector<PreBlock> blocks;
std::deque<std::future<std::shared_ptr<BlockTask>>> pipeline;
uint32_t good_matches_count = 0; uint32_t fail_count = 0;
...
pipeline.push_back(pool.enqueue([task, m_ids, opt_levels, opt_auto, opt_force, &detected_auto_level, &opts]() {
...
int32_t levels_to_check[9]; int num_levels = 0;
if (!opt_levels.empty()) {
...
int64_t res = OodleLZ_Compress(method, task->dec_data.data(), task->usize, local_temp.data(), test_level, &opts, nullptr, nullptr, 0);
