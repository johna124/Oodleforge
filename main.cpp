...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::cout << "=================================================================================\n"
                  ...
                  << "  -m <alg>   Set compressor(s). Supports multiple: kraken+leviathan\n"
                  << "             Options: kraken/8 (default), leviathan/9, mermaid/11, selkie/12.\n"
    ...
    bool verbose = false, debug_mode = false, opt_auto = false, opt_force = false;
    ...
    if (op == "scan") return handle_result(RunScan(input, verbose, num_threads, m_ids, aesKey, useAES, scan_percent, debug_mode));
    if (op == "e") return handle_result(RunEncode(input, output, verbose, num_threads, m_ids, opt_levels, opt_auto, opt_force, aesKey, useAES));
    if (op == "r") return handle_result(RunReconstruct(input, output, verbose, num_threads, aesKey, useAES));
