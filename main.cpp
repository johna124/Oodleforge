...
thread_local int wine_tls_workaround_dummy = 0;
...
int main(int argc, char** argv) {
    ...
    if (argc >= 2) {
        ...
        if (first_arg == "-h" || first_arg == "--help") {
            std::cout << "=================================================================================\n"
                      ...
                      << "             Options: kraken/8, leviathan/11, mermaid/9, selkie/12, hydra/13.\n"
    ...
    if ((op == "e" || op == "r") && output.empty()) {
        std::cerr << "[FATAL] Operation '" << op << "' requires an output file.\n";
        return static_cast<int>(ErrorCode::ERR_INVALID_ARGUMENT);
    }
  ...
  if (op == "e") {
    ...
    if (op == "scan") return handle_result(RunScan(input, verbose, num_threads, m_ids, opt_levels, opt_auto, opt_force, aesKey, useAES, scan_percent, debug_mode, tradeoff_bytes, quantum_crc));
